...
class Planner:
    ...
    def __init__(self,
        ...
        # Local LLM configuration (priority: env var > constructor/config > default)
        # api_base is an OpenAI-compatible base URL that already includes the version, e.g. http://localhost:8000/v1
        self.llm_api_base = (os.getenv("LLM_API_BASE") or api_base or "http://localhost:8000/v1").rstrip("/")
        self.llm_model = os.getenv("LLM_MODEL") or model or "meta-llama/Llama-3.2-1B"
        self.llm_api_key = os.getenv("LLM_API_KEY") or api_key or "DUMMY"
