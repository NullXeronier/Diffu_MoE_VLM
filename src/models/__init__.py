...
class GoalEmbedding:
    ...
    def _hash_to_embedding(self, text: str) -> np.ndarray:
        ...
        # Use a stable hash (built-in hash() is salted per process) to generate a consistent embedding
        seed = int.from_bytes(hashlib.sha256(text.encode("utf-8")).digest()[:8], "little")
        
        # Convert to vector using a local RNG so the global numpy RNG state is untouched
